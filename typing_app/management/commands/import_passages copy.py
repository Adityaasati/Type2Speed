...
class Command(BaseCommand):
    help = 'Import passages (English, Hindi, German, French) and duration into ExamContent'
    ...
    def handle(self, *args, **kwargs):
        ...
        with open(file_path, mode='r', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                passage_german = row['passage_german']  # German passage
                passage_french = row['passage_french']  # French passage
                ...
                if passage_german:
                    passage_german = passage_german.strip('"""').replace('"""', '').replace('"', '').strip()

                if passage_french:
                    passage_french = passage_french.strip('"""').replace('"""', '').replace('"', '').strip()
                ...
                # Create ExamContent with the passages (English, Hindi, German, French) and duration
                exam_content = ExamContent.objects.create(
                    ...
                    passage_german=passage_german if passage_german else None,
                    passage_french=passage_french if passage_french else None,
                    duration=duration,
                )
